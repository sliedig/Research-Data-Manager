...
namespace Urdms.Dmp.Tests.Helpers
{
    ...
    public class ControllerResultTest<T> where T : Controller
    {
        ...
        public void ShouldRedirectTo(MethodInfo method)
        {
            ValidateActionReturnType<RedirectToRouteResult>();
        ...
        /*public void ShouldRedirectTo<TController>(Func<TController, Func<object, ActionResult>> actionRedirectedTo) where TController : Controller
        {
            ShouldRedirectTo<TController>(actionRedirectedTo(_controller).Method);
        }

        public void ShouldRedirectTo<TController>(Func<TController, Func<object, object, ActionResult>> actionRedirectedTo) where TController : Controller
        {
            ShouldRedirectTo<TController>(actionRedirectedTo(_controller).Method);
        }

        public void ShouldRedirectTo<TController>(Func<TController, Func<object, object, object, ActionResult>> actionRedirectedTo) where TController : Controller
        {
            ShouldRedirectTo<TController>(actionRedirectedTo(_controller).Method);
        }
        ...
        {
            var result = _actionResult as EmptyResult;
            if (result == null)
            {
                throw new ActionResultAssertionException(string.Format("Expected empty result but instead was given '{0}'", _actionResult.GetType().Name));
            }
        }
        ...
        public void ShouldRenderContentResult(string expectedContent = null)
        {
            ValidateActionReturnType<ContentResult>();

            var viewResult = (ContentResult)_actionResult;

            if (expectedContent != null && viewResult.Content != expectedContent)
            {
                throw new ActionResultAssertionException(string.Format("Content does not matched expectedContent: {0} != {1}", viewResult.Content, expectedContent));
            }
    ...
    public class ModelTest<TModel>
    {
        ...
        public void AndNoModelErrors()
        {
            if (!_controller.ModelState.IsValid)
                throw new ViewResultModelAssertionException(string.Format("Expected controller '{0}' to have no model errors, but it hadsome.", _controller));
        }
    }
